...
class ProducerTest {
  ...
  void shouldSendMessageToKafkaSuccessfully() {
    ...
    records.iterator().forEachRemaining(
            record -> {
              BookRatingRequest message;
              try {
                message = objectMapper.readValue(record.value(), BookRatingRequest.class);
              } catch (JsonProcessingException e) {
                throw new RuntimeException(e);
              }
              assertEquals(new BookRatingRequest(56L), message);
            }
    );
  }
