...
public class CircuitBreakerTest extends DataBaseSuite {
  ...
  @Test
  void shouldRejectRequestAfterFirstServerFailResponse() throws InvalidDataException {
    var uuid = UUID.randomUUID().toString();
    HttpHeaders headers = new HttpHeaders();
    headers.add("X-REQUEST-ID", uuid);
    var author = authorService.createAuthor("Лев", "Толстой");
    when(restTemplate.exchange(
            ...
            new HttpEntity<>(
                    new AuthorRegistryRequest(
                            author.getFirstName(), author.getLastName(), "Война и мир"),
                    headers),
            AuthorRegistryResponse.class)).thenThrow(new RestClientException("Война и мир"));
    assertThrows(
            CreateBookException.class,
            () -> bookService.createBook("Война и мир", author.getAuthorId(), uuid)
    );
